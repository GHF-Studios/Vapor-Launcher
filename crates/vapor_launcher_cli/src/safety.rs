...
use vapor_launcher_core::{
    CommandSpec, GlobalOptions, LauncherCommand, LauncherInstallCommand, PackCommand,
    PackagepackCommand, RepairCommand,
};
...
pub(crate) fn guard(
    ...
    if globals.strict && !policy.strict {
        return Err(Box::new(SafetyError::UnsupportedFlag {
            flag: "--strict",
            action: spec.action.clone(),
            reason: "this command does not perform validity-tolerant pack mutation",
        }));
    }
...
impl SafetyPolicy {
    fn for_command(command: &LauncherCommand) -> Self {
        match command {
            LauncherCommand::Repair(RepairCommand::Apply { .. }) => Self {
                force: true,
                strict: false,
                keep_unused_versions: true,
                confirm: true,
            },
            LauncherCommand::Packagepack(PackagepackCommand::Installed(command))
            | LauncherCommand::Pack {
                command: PackCommand::Installed(command),
                ..
            }
            | LauncherCommand::Leaf {
                command: vapor_launcher_core::LeafCommand::Installed(command),
                ..
            } => Self::for_install_command(command),
            LauncherCommand::Packagepack(PackagepackCommand::Compose(_))
            | LauncherCommand::Pack {
            ...
            LauncherCommand::Packagepack(PackagepackCommand::Lock { .. })
            | LauncherCommand::LockSelectedPackagepack => Self {
                force: false,
                strict: false,
                keep_unused_versions: true,
                confirm: false,
    ...
    fn for_install_command(command: &LauncherInstallCommand) -> Self {
        match command {
            LauncherInstallCommand::Install { .. } => Self {
                force: false,
                strict: false,
                keep_unused_versions: false,
                confirm: true,
            },
            LauncherInstallCommand::Uninstall { .. } => Self {
                force: true,
                strict: false,
                keep_unused_versions: false,
                confirm: true,
            },
            LauncherInstallCommand::Update { .. } => Self {
                force: true,
                strict: false,
                keep_unused_versions: true,
                confirm: true,
